...
const Debounce = () => {
  ...
  let handleChange = (e)=>{
    setState(true)
    setTimeout(()=>{
      let newData = data.filter((obj)=>obj.quote.includes(e.target.value))
      setFiltered(newData)
      setState(false)
    },1000)
  }
  ...
  return (
    <Fragment>
      <nav style={{display:"flex",alignItems:"center",height:"10vh",backgroundColor:"gainsboro", flexDirection:"column"}}>
        <input style={{width:"90%", height:"7vh", borderRadius:"10px",paddingLeft:"20px"}} type="text"  placeholder='Search Quote' onChange={handleChange}/>
      ...
      <div style={{display:"flex",flexWrap:"wrap"}}>
        {
          filtered.map((obj)=>{
            return(
              <div key={obj.id} style={{width:"23.5%",margin:"10px",padding:"10px",backgroundColor:"gainsboro"}}>
                <p>{obj.quote}</p>
                <p>~ <span style={{fontWeight:"bold"}}>{obj.author}</span>{obj.id} </p>
                <br />
              </div>
            )
          })
